...
from .models import Order, db #імпортуванння моделі заказа для взаємодіЇ з таблицею данних
...
def render_cart():
    if flask.request.method == 'POST':
        if flask.request.form.get('form-order') != None:
            ...
            try:
                ...
                text_message = flask_mail.Message(
                    subject=flask.request.form["email"],
                    recipients=[flask.request.form["email"]],
                    sender="arturkakish5@gmail.com"
                )
                print("success")
...
def render_cart_process():
    if flask.request.method == 'POST':
        print("canceled")
    return flask.render_template(template_name_or_list="cart_process.html", login=User.query.all())